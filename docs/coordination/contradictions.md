...
| C9 | Validation gate expectation vs repository baseline | Run policy requires green `cargo fmt` / `clippy -D warnings` / `cargo test` / `npm run build` before/after steps | 2026-07-26 run shows pre-existing failures in all four gates (fmt drift, clippy `uninlined_format_args`, `auth_session_audit_tests` quota failure, TS6133 build errors) | **Open blocker** — quarantine baseline failures or remediate in dedicated follow-up before claiming fully green matrix. |
