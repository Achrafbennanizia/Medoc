...
**Operational finding:** npm workspace installs must include workspace flags for this repo (`npm ci --workspaces --include-workspace-root`), otherwise shared workspace dependencies (for example `i18next`) are not fully materialized for checks.
