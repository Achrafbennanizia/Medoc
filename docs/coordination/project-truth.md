...
- **CI/CD scope (2026-07-09):** `.github/workflows/{verify,autofix,fix-proposal,release}.yml` — tiered pipeline with verify-only gate, PR-only deterministic autofix, draft fix-proposal PR flow, and release gate using reusable `verify.yml` plus protected `release` environment for signed artifacts.
