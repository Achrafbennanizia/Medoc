...
- **Workflow logging bridge (2026-07-09):** frontend route/IPC lifecycle emits `workflow_log_event` to Rust command; dedicated `workflow.log` target `medoc::workflow` uses sanitized writer and daily rolling appender (`apps/practice-host-ui/src/services/tauri.service.ts`, `workflow-route-logger.tsx`, `crates/app/medoc-practice/src/commands/system/logging.rs`, `crates/shared/medoc-core/src/infrastructure/logging/mod.rs`).
- **UI audit harness (2026-07-09):** `/qa/ui-audit` route + Playwright spec validate spacing/a11y/keyboard/toast policy at 375/768/1259 with snapshot baselines in `e2e-playwright/ui-geometry-spacing.spec.ts-snapshots/`.
