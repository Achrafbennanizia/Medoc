...
- **Workflow telemetry channel (2026-07-25):** shared tracing now includes dedicated `workflow.log` (`medoc::workflow` target) and frontend route/action lifecycle bridge via `log_workflow_event` IPC with sanitizer label scrubbing (`crates/shared/medoc-core/src/infrastructure/logging/{mod.rs,sanitizer.rs}`, `crates/app/medoc-practice/src/commands/system/logging.rs`, `apps/practice-host-ui/src/services/tauri.service.ts`).
