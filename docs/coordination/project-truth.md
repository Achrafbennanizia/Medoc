...
- **CI/CD scope (2026-08-25):** Tiered workflows under `.github/workflows/` — `verify.yml` (non-mutating Rust/web/a11y gate), `autofix.yml` (deterministic PR-only fixes + loop guard + sensitive-path mutation block), `fix-proposal.yml` (draft PR remediation flow with evidence + sensitive-path human-review label), `release.yml` (verify gate reuse + signed cross-platform build in protected `release` environment). Legacy monolithic `ci.yml` retired.
