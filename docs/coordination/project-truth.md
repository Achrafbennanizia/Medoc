...
- **CI/CD scope:** tiered workflows under `.github/workflows/` — `verify.yml` (push/PR/workflow_call, zero mutation), `autofix.yml` (pull_request deterministic fixes only), `fix-proposal.yml` (draft PR proposals on manual dispatch or failed `verify` on `main`), `release.yml` (tag/dispatch, verify-gated signed build in protected `release` environment).
...
| CI/CD workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-07-12 |
...
| CI without next-web | `.github/workflows/verify.yml` | 2026-07-12 |
