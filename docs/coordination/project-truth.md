...
- **CI/CD scope (2026-08-25):** tiered workflows — `verify.yml` (blocking zero-mutation checks: Rust fmt/clippy/test/audit + web lint/typecheck/test/build + a11y), `autofix.yml` (PR-only deterministic fixes with bot loop guard), `fix-proposal.yml` (draft PR proposals on new branches with sensitive-path label stop), and `release.yml` (reusable verify gate + protected signed cross-platform bundles). **No** root-level `src/` Next.js tree in repo (2026-05-19).
...
| CI/CD tiered workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-08-25 |
...
| CI without next-web | `.github/workflows/verify.yml` | 2026-08-25 |
