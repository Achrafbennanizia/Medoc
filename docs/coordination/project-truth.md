...
- **Workflow observability channel (2026-07-26):** tracing subsystem now includes dedicated `workflow.log` (`medoc::workflow`) with sanitized frontend bridge command `log_workflow_event` and route/IPC/domain transition emitters (`logging/mod.rs`, `commands/system/logging.rs`, `workflow-log.ts`, `workflow_transitions.rs`).
