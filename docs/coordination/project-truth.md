...
- **CI/CD scope (2026-07-25):** tiered workflows in `.github/workflows/{verify,autofix,fix-proposal,release}.yml`. `verify.yml` is zero-mutation gate (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + axe-core critical scan), `autofix.yml` is PR-only deterministic fixer, `fix-proposal.yml` opens draft PR proposals, and `release.yml` reuses verify as release gate with protected environment.
...
| CI commands | `.github/workflows/verify.yml`, `.github/workflows/autofix.yml`, `.github/workflows/fix-proposal.yml`, `.github/workflows/release.yml` | 2026-07-25 |
...
| Legacy CI retired (`next-web` already removed earlier) | `.github/workflows/ci.yml` deleted; tier split in `verify.yml` | 2026-07-25 |
