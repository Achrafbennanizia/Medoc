...
- **CI/CD scope (2026-08-26):** tiered workflows under `.github/workflows/`: `verify.yml` (blocking verify), `autofix.yml` (PR deterministic fixes only), `fix-proposal.yml` (draft PR proposals on demand / failed main verify), `release.yml` (verify-gated signed release build under protected `release` environment). Legacy `.github/workflows/ci.yml` retired.
...
| CI/CD tier workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-08-26 |
...
| Legacy CI without next-web (historical) | `.github/workflows/ci.yml` | 2026-05-19 |
