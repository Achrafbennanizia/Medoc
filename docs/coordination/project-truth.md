...
- **CI/CD scope:** Tiered workflows in `.github/workflows/{verify,autofix,fix-proposal,release}.yml` — verify is check-only (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + axe-core); autofix is PR-only deterministic formatting/lint; fix-proposal opens draft PRs on new branches; release reuses verify and builds signed artifacts in protected `release` environment.
...
| CI/CD workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-07-25 |
