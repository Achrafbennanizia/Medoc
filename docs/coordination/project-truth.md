...
- **CI/CD scope (2026-08-26):** Tiered workflows under `.github/workflows/` — `verify.yml` (Rust fmt/clippy/test/audit + web lint/typecheck/test/build + a11y), `autofix.yml` (deterministic PR-only fixes), `fix-proposal.yml` (draft PR proposals on manual/failed-main), `release.yml` (verify gate + protected signed build + provenance). `ci.yml` is a compatibility wrapper that delegates to `verify.yml`.
