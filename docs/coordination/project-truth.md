...
- **CI/CD scope (2026-06-25):** tiered workflows in `.github/workflows/{verify,autofix,fix-proposal,release}.yml` with a legacy `ci.yml` redirect. Tier 1 verify is immutable (Rust fmt/clippy/test/audit + web lint/typecheck/test/build + axe-core critical WCAG gate). Tier 2 autofix is PR-only and deterministic. Tier 3 opens draft fix-proposal PRs. Tier 4 release reruns verify and builds signed artifacts in protected `release` environment.
...
| CI/CD workflow tiers | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-06-25 |
