...
- **CI/CD scope:** tiered workflows at `.github/workflows/{verify,autofix,fix-proposal,release}.yml`. `verify.yml` is non-mutating (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + axe-core check); `autofix.yml` is PR-only deterministic formatting/linting; `fix-proposal.yml` opens draft PRs for substantive fixes; `release.yml` re-runs verify and builds signed artifacts behind protected `release` environment.
...
| CI/CD workflows | `.github/workflows/verify.yml`, `.github/workflows/autofix.yml`, `.github/workflows/fix-proposal.yml`, `.github/workflows/release.yml` | 2026-08-26 |
...
| Legacy CI removal | `.github/workflows/ci.yml` deleted; tiered workflows active | 2026-08-26 |
