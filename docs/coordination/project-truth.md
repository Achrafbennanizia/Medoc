...
- **CI scope (tiered, 2026-08-26):** `.github/workflows/verify.yml` (blocking verify), `.github/workflows/autofix.yml` (PR-only deterministic fixes), `.github/workflows/fix-proposal.yml` (draft PR proposals for substantive fixes), `.github/workflows/release.yml` (gated signed release via protected environment). Lockfile-based package-manager detection supports pnpm/yarn/npm; active JS lockfile is `package-lock.json`.
...
| CI workflows | `.github/workflows/verify.yml`, `autofix.yml`, `fix-proposal.yml`, `release.yml` | 2026-08-26 |
