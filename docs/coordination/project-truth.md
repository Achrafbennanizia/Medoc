...
- **CI/CD scope:** Tiered workflows under `.github/workflows/` — `verify.yml` (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + axe critical WCAG gate), `autofix.yml` (PR-only deterministic fixes), `fix-proposal.yml` (draft PR proposals for non-deterministic fixes), and `release.yml` (verify gate + protected signed cross-platform build). Legacy `ci.yml` removed during workspace-path migration (2026-08-25).
...
| CI/CD commands | `.github/workflows/verify.yml`, `.github/workflows/autofix.yml`, `.github/workflows/fix-proposal.yml`, `.github/workflows/release.yml` | 2026-08-25 |
