...
- **CI/CD scope (2026-06-25):** tiered workflows under `.github/workflows/` — `verify.yml` (blocking verify + a11y), `autofix.yml` (PR deterministic fixes only), `fix-proposal.yml` (draft PR proposals on failed verify/manual trigger), `release.yml` (verify gate + protected signed release build + provenance). Stale `ci.yml` removed.
...
| CI/CD workflow tiers | `.github/workflows/{verify.yml,autofix.yml,fix-proposal.yml,release.yml}` | 2026-06-25 |
