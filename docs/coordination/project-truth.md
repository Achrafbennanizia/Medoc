...
- **CI/CD tiering (2026-06-26):** `.github/workflows/verify.yml` (blocking verify), `autofix.yml` (PR-only deterministic fixes with bot loop guard), `fix-proposal.yml` (manual/failed-main draft PR proposals), and `release.yml` (verify gate + protected signed release). Legacy `.github/workflows/ci.yml` removed.
