...
- **Workflow telemetry channel (2026-07-26):** Logging stack includes dedicated `workflow.log` target (`medoc::workflow`), backend IPC bridge `log_workflow_event`, and frontend emitters for route-enter + Tauri command lifecycle (`apps/practice-host-ui/src/services/tauri.service.ts`, `packages/app/practice-host/src/lib/workflow-logger.ts`).
