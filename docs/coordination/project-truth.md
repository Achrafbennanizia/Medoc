...
- **CI/CD scope (2026-08-26):** tiered workflows in `.github/workflows/{verify.yml,autofix.yml,fix-proposal.yml,release.yml}`. Tier 1 verify is mutation-free (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + a11y critical gate), tier 2 autofix is PR-only deterministic formatting/lint, tier 3 opens draft fix-proposal PRs, tier 4 release reuses verify then builds signed artifacts behind protected `release` environment. **No** root-level `src/` Next.js tree in repo.
