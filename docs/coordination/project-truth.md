...
- **CI/CD scope (2026-07-26):** tiered workflows under `.github/workflows/` — `verify.yml` (read-only push/PR gate, rust + web + axe), `autofix.yml` (PR-only deterministic `fmt`/`lint:fix`/`format`), `fix-proposal.yml` (manual or failed-main draft PR proposal with evidence), and `release.yml` (tag/dispatch, re-verify + signed build in protected `release` environment). Legacy `ci.yml` has been retired.
