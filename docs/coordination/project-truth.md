...
- **CI/CD scope:** tiered GitHub Actions workflows — `verify.yml` (blocking verify + a11y, zero mutation), `autofix.yml` (deterministic PR-only fixes), `fix-proposal.yml` (draft PR proposals for failing main/manual dispatch), `release.yml` (tag/dispatch verify-gated signed builds under protected environment). **No** root-level `src/` Next.js tree in repo (2026-05-19).
...
| CI/CD workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-07-26 |
