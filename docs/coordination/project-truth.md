...
- **CI/CD scope:** tiered workflows under `.github/workflows/`: `verify.yml` (Rust/Web/a11y zero-mutation gate), `autofix.yml` (deterministic PR-only fixes with loop guard), `fix-proposal.yml` (draft PR proposals on manual dispatch or failed `main` verify), and `release.yml` (verify gate + signed release build in protected `release` environment). Legacy `ci.yml` was removed in favor of the four-tier split (2026-08-25).
