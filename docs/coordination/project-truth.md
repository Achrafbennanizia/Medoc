...
- **CI scope:** `.github/workflows/verify.yml` (push/PR/workflow_call verify gate), `.github/workflows/autofix.yml` (PR-only deterministic fixes), `.github/workflows/fix-proposal.yml` (draft PR proposals for non-deterministic fixes), `.github/workflows/release.yml` (tag/dispatch release with verify gate + signed artifacts). **No** root-level `src/` Next.js tree in repo (2026-05-19).
