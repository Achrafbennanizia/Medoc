...
- **CI/CD scope (2026-08-26):** tiered workflows under `.github/workflows/` — `verify.yml` (push/PR/workflow_call, non-mutating Rust+JS+a11y gates), `autofix.yml` (PR-only deterministic formatting/lint fixes with loop + compliance guards), `fix-proposal.yml` (manual/red-main draft-PR proposals with before/after evidence), `release.yml` (tag/dispatch, verify gate reuse, signed cross-platform bundles in protected `release` environment). `ci.yml` is now a thin legacy wrapper that delegates to `verify.yml`.
...
| CI/CD tier workflows | `.github/workflows/{verify,autofix,fix-proposal,release,ci}.yml`, `docs/coordination/ci-cd-plan.md` | 2026-08-26 |
...
| CI without next-web | `.github/workflows/ci.yml` (legacy wrapper; no next-web job) | 2026-08-26 |
