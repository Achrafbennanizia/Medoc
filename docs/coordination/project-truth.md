...
- **CI/CD scope (tiered):** `.github/workflows/verify.yml` (Rust/Web/a11y verify, zero mutation), `autofix.yml` (deterministic PR-only fixes), `fix-proposal.yml` (draft PR proposals for substantive fixes), `release.yml` (verify gate + signed protected release build). Legacy monolithic `.github/workflows/ci.yml` is retired.
