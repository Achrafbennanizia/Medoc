...
- **CI scope:** `.github/workflows/{verify,autofix,fix-proposal,release}.yml` — tiered pipeline: non-mutating verify (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + axe-core critical a11y gate), PR-only deterministic autofix, draft fix proposals for failed `main` verify/manual dispatch, and gated signed release builds. **No** root-level `src/` Next.js tree in repo (2026-08-26).
