...
- **CI/CD scope (2026-07-26):** Four-tier pipeline under `.github/workflows/`: `verify.yml` (blocking verify), `autofix.yml` (PR-only deterministic fixes), `fix-proposal.yml` (draft PR proposals for substantive fixes), `release.yml` (manual-approval gated signed release). `verify.yml` targets the live root workspaces (`apps/*`, `crates/*`, `packages/*`) and no longer relies on retired `app/src-tauri` assumptions.
...
| CI/CD pipeline tiers | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-07-26 |
...
| CI without next-web | `.github/workflows/verify.yml` | 2026-07-26 |
