...
- **CI scope:** tiered workflows under `.github/workflows/` — `verify.yml` (fmt/clippy/test/audit + JS lint/typecheck/test/build + axe-core), `autofix.yml` (deterministic PR-branch fixes only), `fix-proposal.yml` (draft PR proposals for non-deterministic fixes), `release.yml` (verify-gated signed build in protected `release` env). `MEDOC_VENDOR_PUBKEY` remains required for Rust checks. **No** root-level `src/` Next.js tree in repo (2026-05-19).
