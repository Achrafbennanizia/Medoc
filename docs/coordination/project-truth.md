...
- **CI/CD scope (2026-07-26):** Tiered workflows under `.github/workflows/` — `verify.yml` (non-mutating Rust/web/a11y gate on `main` push + PR), `autofix.yml` (deterministic PR-branch-only formatter/lint fixes), `fix-proposal.yml` (draft PR remediation flow on manual dispatch or failed `verify` on `main`), and `release.yml` (verify-gated signed matrix build behind protected `release` environment). `ci.yml` is now a manual legacy wrapper delegating to `verify.yml`.
