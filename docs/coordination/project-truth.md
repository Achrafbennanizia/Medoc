...
- **CI/CD scope (2026-07-09):** `.github/workflows/verify.yml` (blocking verify), `.github/workflows/autofix.yml` (deterministic PR-branch fixes only), `.github/workflows/fix-proposal.yml` (draft proposal PRs for non-deterministic failures), `.github/workflows/release.yml` (tag/dispatch gated signed build under protected `release` environment). **No** root-level `src/` Next.js tree in repo (2026-05-19).
...
| CI commands | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-07-09 |
...
| CI without next-web | `.github/workflows/verify.yml` (Rust/Web/A11y only) | 2026-07-09 |
