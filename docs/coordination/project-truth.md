...
- **CI/CD scope (2026-08-26):** Tiered workflows in `.github/workflows/` — `verify.yml` (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + axe-core critical a11y gate), `autofix.yml` (PR-only deterministic fixes), `fix-proposal.yml` (draft PR remediation proposals), and `release.yml` (verify-gated signed matrix build under protected `release` env). `ci.yml` is now a compatibility wrapper to reusable `verify.yml`.
