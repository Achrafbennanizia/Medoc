...
- **CI/CD scope (2026-07-26):** Tiered workflows under `.github/workflows/` — `verify.yml` (blocking, zero-mutation Rust/Web/A11y), `autofix.yml` (deterministic PR-only fixes), `fix-proposal.yml` (draft PR proposals with evidence + sensitive-path stop), `release.yml` (verify-gated signed builds in protected `release` environment). Legacy `ci.yml` retired.
