...
- **CI/CD scope:** tiered workflows under `.github/workflows/` — `verify.yml` (non-mutating gate), `autofix.yml` (PR-only deterministic fixes), `fix-proposal.yml` (draft PR proposal flow), `release.yml` (verify + protected signed release). Legacy `ci.yml` was retired on 2026-08-26.
...
| CI/CD tier commands | `.github/workflows/{verify.yml,autofix.yml,fix-proposal.yml,release.yml}` | 2026-08-26 |
