...
- **CI/CD scope (tiered):** `.github/workflows/verify.yml` (push/PR/workflow_call, non-mutating), `.github/workflows/autofix.yml` (PR-only deterministic fixes + loop guard), `.github/workflows/fix-proposal.yml` (new-branch draft PR proposals), `.github/workflows/release.yml` (tag/dispatch, verify gate + protected `release` env + signed bundles). `ci.yml` is now a `workflow_call` compatibility wrapper to `verify.yml`. No root-level `src/` Next.js tree in repo.
...
| CI/CD tiered workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml`, `ci.yml` wrapper | 2026-07-26 |
