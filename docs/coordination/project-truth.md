...
- **CI/CD scope:** four-tier workflow set under `.github/workflows/`: `verify.yml` (blocking verify, no mutation), `autofix.yml` (deterministic PR-only fixes), `fix-proposal.yml` (draft PR remediation branch), `release.yml` (verify-gated signed release build in protected `release` environment). **No** root-level `src/` Next.js tree in repo (2026-05-19).
