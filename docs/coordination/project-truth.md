...
- **G21 Posteingang:** Route `/inbox`, RBAC, sidebar (`lib/nav-sections.ts`), badge polling, native Go menu — **verified in code**; live Tauri walkthrough **NOT OBSERVED** (`docs/coordination/g21-live-smoke-checklist.md`). Automated gate: `bash tools/g21-verify-automated.sh`.
...
- **AMVV prescription/certificate:** Extended columns via migrations in `connection.rs`; round-trip tests in `db_migrations_tests.rs`; edit UI in `prescription-edit.tsx`.
...
- **Outbox hooks (Wave V1):** Repo write paths in `medoc-core::infrastructure::database::{patient,akte,appointment,payment,practice_task,app_kv}_repo` call `sync_outbox::record_or_noop` which appends one row to `sync_outbox` when `mode = serverless_peer` and the table is in `SYNCED_TABLES`. Internal `sync.*`, `license.*`, `pairing.*` `app_kv` keys are excluded. 7 integration tests + 3 unit tests cover this path.
...
- **Traceability matrix:** `docs/requirements-engineering/01b-traceability-waad.md` maps every
  WAAD-ID onto one or more `FA-*` / `NFA-*` IDs in `docs/version-model/01-anforderungen/pflichtenheft.md`,
...
- **Implementation status of new IDs (2026-05-21 gap remediation):** See `docs/coordination/actions.md`.
  **Done in code:** `FA-AKTE-14/15/16`, `FA-DOK-08`, `FA-PERS-07/08`, `NFA-SEC-08` (SQLCipher), `NFA-USE-10`,
  restore backup (WAAD 9.1), Krankheitsbild statistics proxy (WAAD 9.5 / A10).
  **Done:** `FA-LEIST-05` (B/U `freigegeben_*` + `pricing::require_released_for_billing`; FE `billing-release.ts`; docs rescoped G13).
  **Done:** `FA-LEIST-06` (Behandlung), `FA-LEIST-07` (Untersuchung: `category`/`service_name`/`total_cost`, `ensure_open_booking_for_billable_untersuchung`, `UntersuchungBillingFields`, `payment-buchung` Soll).
  **Done:** `FA-AUFG-01..06` (`practice_task`, `/inbox`, Statusmaschine, `PRAXIS_AUFGABE_ERLEDIGT`, auto `ABRECHNUNG`, manueller Dialog „Aufgabe an Rezeption“ in Patientenakte). Legacy `/tickets` + `practice_ticket` IPC bleiben parallel.
...
| WAAD-Pflichtenheft delta | `docs/version-model/01-anforderungen/pflichtenheft.md` (FA-AKTE-14..16, FA-DOK-08, FA-LEIST-05, FA-PERS-07/08, NFA-USE-09/10) | 2026-04-25 |
