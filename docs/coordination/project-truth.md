...
- **Workflow logging channel (2026-08-26):** tracing file stack now includes `workflow.log` (`Rotation::DAILY`) and all file sinks pass through sanitizer-at-write wrappers (`SanitizingMakeWriter`/`SanitizingWriter`) in `crates/shared/medoc-core/src/infrastructure/logging/mod.rs`; `app.log` filter explicitly excludes `medoc::workflow` in `config.rs`.
- **Frontend→backend workflow bridge (2026-08-26):** UI route/action lifecycle events (`route_enter`, `primary_action`, `success`, `cancel`, `error`) are emitted from `apps/practice-host-ui/src/services/workflow-logging.service.ts` + `tauri.service.ts` to Tauri command `log_workflow_event` (`crates/app/medoc-practice/src/commands/system/logging.rs`).
- **Domain transition observability (2026-08-26):** state machine transitions for Termin, Patientenakte, Praxis-Ticket, Praxis-Aufgabe, and Bestellung now emit structured workflow attempt/outcome logs via `log_workflow!` in `crates/shared/medoc-core/src/domain/services/workflow_transitions.rs`.
