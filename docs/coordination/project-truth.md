...
- **Workflow observability (2026-06-25):** logging subsystem includes dedicated `workflow.log` channel (`medoc::workflow`) plus frontend bridge command `log_workflow_event`; route-enter and Tauri command lifecycle events are emitted through sanitizer-backed logging (`crates/shared/medoc-core/src/infrastructure/logging/mod.rs`, `crates/app/medoc-practice/src/commands/system/logging.rs`, `apps/practice-host-ui/src/services/tauri.service.ts`, `apps/practice-host-ui/src/App.tsx`).
