...
- **CI/CD scope:** Tiered workflows under `.github/workflows/` — `verify.yml` (blocking verify), `autofix.yml` (deterministic PR-branch fixes), `fix-proposal.yml` (draft PR remediation), `release.yml` (re-verify + signed gated release). Package manager is lockfile-detected; verify remains non-mutating.
