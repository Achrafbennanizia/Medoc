...
- **Workflow telemetry channel (2026-06-25):**
  - Backend logging includes dedicated `workflow.log` daily appender and `medoc::workflow` target filtering (`crates/shared/medoc-core/src/infrastructure/logging/mod.rs`, `config.rs`).
  - Tauri command `workflow_log_step` is registered in invoke handler (`crates/app/medoc-practice/src/commands/system/logging.rs`, `commands/register.rs`).
  - Frontend bridge emits route/action/success/error workflow events via `workflow-log-events.ts`, `tauri.service.ts`, `app-layout.tsx`, and dialog cancel hooks in `packages/ui/src/dialog.tsx`.
