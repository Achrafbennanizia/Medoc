...
- **CI scope:** Tiered workflows: `verify.yml` (blocking Rust + JS + a11y), `autofix.yml` (deterministic PR-branch fixes only), `fix-proposal.yml` (draft PR proposals), `release.yml` (verify-gated signed builds under protected release approval). `verify.yml` targets the live workspace layout (`apps/*`, `crates/*`, `packages/*`). **No** root-level `src/` Next.js tree in repo (2026-05-19).
...
| CI commands | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-08-26 |
