...
- **CI/CD scope (2026-07-26):** tiered workflows in `.github/workflows/{verify,autofix,fix-proposal,release}.yml`. `verify.yml` is the blocking gate (Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + a11y). `autofix.yml` is PR-only deterministic formatting/lint fix with bot loop guard. `fix-proposal.yml` opens draft PR proposals for failed `verify` on `main`. `release.yml` re-runs `verify` and builds signed artifacts under protected `release` environment. **No** root-level `src/` Next.js tree in repo (2026-05-19).
...
| CI/CD workflows | `.github/workflows/verify.yml`, `.github/workflows/autofix.yml`, `.github/workflows/fix-proposal.yml`, `.github/workflows/release.yml` | 2026-07-26 |
