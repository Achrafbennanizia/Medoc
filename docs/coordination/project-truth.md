...
- **CI scope:** tiered workflows under `.github/workflows/{verify,autofix,fix-proposal,release}.yml`. `verify.yml` runs Rust fmt/clippy/test/audit plus web lint/typecheck/test/build and axe-core critical WCAG 2.1 AA scan; `autofix.yml` applies deterministic PR-branch-only fixes with loop guard; `release.yml` reuses `verify.yml` as gate and builds signed artifacts in protected `release` environment.
...
| CI commands | `.github/workflows/verify.yml` | 2026-07-09 |
...
| CI without next-web | `.github/workflows/verify.yml` | 2026-07-09 |
