...
- **CI/CD scope:** tiered workflows under `.github/workflows/` — `verify.yml` (fmt/clippy/test/audit + JS lint/typecheck/test/build + axe), `autofix.yml` (deterministic PR autofix only), `fix-proposal.yml` (draft PR proposals on red `main`/manual), `release.yml` (re-verify + signed build in protected `release` environment). **No** root-level `src/` Next.js tree in repo.
...
| CI/CD workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-06-12 |
...
| CI without next-web | `.github/workflows/verify.yml` | 2026-06-12 |
