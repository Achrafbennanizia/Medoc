...
- **CI scope:** tiered workflows under `.github/workflows/` — `verify.yml` (fmt-check, clippy `-D warnings`, tests, audit, lint/typecheck/test/build, critical axe gate), `autofix.yml` (PR-only deterministic fixes + bot loop guard), `fix-proposal.yml` (draft PR proposals), `release.yml` (verify-gated signed builds behind protected `release` environment).
...
| CI commands | `.github/workflows/verify.yml` | 2026-08-26 |
