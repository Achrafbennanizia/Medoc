...
## CI/CD tier migration (2026-07-26) — stable truth

- **Workflow split:** `.github/workflows/ci.yml` was replaced by four tiered workflows:
  `verify.yml`, `autofix.yml`, `fix-proposal.yml`, `release.yml`.
- **Verify is reusable:** `verify.yml` now supports `workflow_call` so `release.yml`
  can gate tagged builds against the same checks.
- **A11y gate wiring:** `scripts/ci/run-axe-critical.mjs` runs axe-core via
  `@axe-core/playwright` and fails on critical WCAG 2.1 A/AA violations.
- **Autofix constraints:** deterministic-only fixes (`cargo fmt`, eslint fix/format),
  PR branch scope, bot loop guard, and protected/release-like branch guard.
- **Fix proposal constraints:** draft-PR-only proposals on isolated branches; sensitive
  changes (`security|audit|crypto|rbac`) receive `needs-human-review`.
...
- **CI/CD scope:** `.github/workflows/verify.yml` (blocking verify), `.github/workflows/autofix.yml` (PR-only deterministic fixes), `.github/workflows/fix-proposal.yml` (draft PR proposals on failed main/manual), `.github/workflows/release.yml` (tag/dispatch release gate + signed build under protected environment). Legacy `.github/workflows/ci.yml` was removed (2026-07-26).
...
| CI/CD tiered workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml`, `scripts/ci/run-axe-critical.mjs` | 2026-07-26 |
...
| Historical removal of next-web job | Prior `.github/workflows/ci.yml` change (retired file) | 2026-05-19 |
