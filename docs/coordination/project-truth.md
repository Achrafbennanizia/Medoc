...
- **CI scope:** Tiered workflows under `.github/workflows/`: `verify.yml` (blocking checks incl. rust fmt/clippy/test/audit, web lint/typecheck/test/build, axe a11y), `autofix.yml` (PR-only deterministic fixes), `fix-proposal.yml` (draft PR remediation branch), and `release.yml` (verify gate + signed build in protected environment). **No** root-level `src/` Next.js tree in repo (2026-05-19).
