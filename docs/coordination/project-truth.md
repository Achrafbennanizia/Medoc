...
- **Workflow observability (2026-08-26):** dedicated `medoc::workflow` logging channel + sanitizer-backed frontend bridge (`log_workflow_event`) + route-enter bridge (`WorkflowRouteBridge`) are implemented in the practice host stack (`crates/shared/medoc-core/src/infrastructure/logging/`, `crates/app/medoc-practice/src/commands/system/logging.rs`, `apps/practice-host-ui/src/services/tauri.service.ts`).
- **Toast placement rule (2026-08-26):** UI toast stack is bottom-right anchored via `.toast-stack { bottom: ...; right: ... }` in `apps/practice-host-ui/src/index.css`, validated by Playwright geometry spec.
- **Frontend test execution (2026-08-26):** workspace `npm test` is orchestrated as three isolated runs (`test:node`, `test:smoke`, `test:mvp-unit`) to avoid process-heap exhaustion in monolithic runs (`apps/practice-host-ui/package.json`).
