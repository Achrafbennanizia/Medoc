...
- **CI/CD scope (tiered):** `verify.yml` (non-mutating gate for Rust/web/a11y on push+PR), `autofix.yml` (deterministic PR-branch fixes only), `fix-proposal.yml` (draft PR proposals for substantive fixes), `release.yml` (reusable verify gate + signed build under protected `release` environment). Legacy `ci.yml` retired (2026-07-09).
...
| CI/CD tiered workflows | `.github/workflows/{verify,autofix,fix-proposal,release}.yml` | 2026-07-09 |
...
| CI without next-web | historical (`next-web` removed; workflow topology now tiered) | 2026-05-19 / 2026-07-09 |
