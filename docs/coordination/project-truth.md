...
- **CI scope:** Tiered workflows in `.github/workflows/{verify,autofix,fix-proposal,release}.yml`; legacy `.github/workflows/ci.yml` retired (2026-07-25). Verify runs Rust fmt/clippy/test/audit + JS lint/typecheck/test/build + axe-core audit; autofix is PR-only deterministic fmt/lint; release reuses verify as gate and builds signed artifacts under protected `release` environment.
