...
- **CI scope (2026-08-26):** Tiered workflows at `.github/workflows/{verify,autofix,fix-proposal,release}.yml`; `verify` is non-mutating gate (Rust + JS + a11y), `autofix` is PR-only deterministic formatter/lint fixer, `fix-proposal` opens draft PRs for non-deterministic fixes, and `release` re-runs full verify before signed artifacts in protected `release` environment.
