...
- **CI/CD scope (2026-06-25):** tiered workflows in `.github/workflows/{verify,autofix,fix-proposal,release}.yml`; legacy `.github/workflows/ci.yml` removed. `verify` is non-mutating (Rust fmt/clippy/test/audit + web lint/typecheck/test/build + axe-core a11y), `autofix` is PR-only deterministic formatting/linting, `fix-proposal` creates draft PRs on new branches, and `release` reuses `verify` then builds signed artifacts under protected `release` environment.
