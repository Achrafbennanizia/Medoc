...
- **CI/CD scope (2026-08-26):** tiered workflows under `.github/workflows/` — `verify.yml` (Rust fmt/clippy/test/audit + web lint/typecheck/test/build + axe-core critical a11y gate), `autofix.yml` (PR-only deterministic fixes), `fix-proposal.yml` (draft proposal PRs), and `release.yml` (verify gate + protected release build). Legacy `ci.yml` is retired.
