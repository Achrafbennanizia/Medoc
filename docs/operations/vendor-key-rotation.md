...
`.github/workflows/verify.yml` and `.github/workflows/release.yml` set `MEDOC_VENDOR_PUBKEY` for Rust verification/build jobs. Local developers must export the same variable (or your org’s production pubkey for release builds).
