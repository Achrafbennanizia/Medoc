...
- CI-Gate: `cargo fmt -- --check`, `cargo check`, `cargo test`, `cargo clippy -- -D warnings`, `cargo audit`, `npm audit --omit=dev`, `npm run lint`, `npm test`, `npm run build` (siehe `.github/workflows/verify.yml`).
