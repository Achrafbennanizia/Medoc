...
- CI-Gate: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `cargo audit`, `npm run lint`, TypeScript type-check, `npm test`, `npm run build`, axe-core critical WCAG 2.1 AA scan (siehe `.github/workflows/verify.yml`).
