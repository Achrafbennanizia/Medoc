...
- CI-Gate: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `cargo audit`, `lint`, `typecheck`, `test`, `build`, and WCAG 2.1 AA a11y checks (siehe `.github/workflows/verify.yml`).
