...
- CI-Gate: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `cargo audit`, Web-Lint/Typecheck/Test/Build, plus axe-core accessibility check (siehe `.github/workflows/verify.yml`).
