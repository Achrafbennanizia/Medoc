...
- CI-Gate: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `cargo audit`, `npm run lint`, `npm run typecheck`, `npm run test`, `npm run build`, `npm run test:a11y` (siehe `.github/workflows/verify.yml`; deterministic PR-autofix in `.github/workflows/autofix.yml`; release gate in `.github/workflows/release.yml`).
