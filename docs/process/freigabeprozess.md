...
- CI-Gate: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace --tests`, `cargo audit`, `npm run lint`, `npm run typecheck`, `npm run test`, `npm run build`, `npm run test:a11y` (siehe `.github/workflows/verify.yml`).
