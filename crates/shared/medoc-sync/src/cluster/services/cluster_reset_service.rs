...
const KEYCHAIN_DEVICE: &str = "cluster-device-signing-key";
...
async fn list_member_targets(pool: &SqlitePool) -> Result<Vec<MemberResetTarget>, AppError> {
    ...
    let all = repos.list_active(&license.cluster_id).await?;
    Ok(all
        .into_iter()
        .filter(|g| {
...
async fn revoke_all_remote_devices(pool: &SqlitePool, actor_user_id: &str) -> Result<(), AppError> {
    ...
    for g in repos.list_active(&license.cluster_id).await? {
...
async fn wipe_network_tables(pool: &SqlitePool) -> Result<(), AppError> {
    ...
    sqlx::query("DELETE FROM device_blocklist")
        .execute(pool)
        .await
        .map_err(AppError::Database)?;
...
/// Wipe license, cluster mesh, onboarding, and device keychain keys.
...
mod tests {
    ...
    async fn wipe_network_kv_clears_license_and_onboarding_keys() {
        ...
        app_kv_repo::set(&pool, "onboarding.setup_complete.v1", "1")
            .await
            .unwrap();
        app_kv_repo::set(&pool, "onboarding.progress.v1.physician", "{}")
            .await
            .unwrap();
