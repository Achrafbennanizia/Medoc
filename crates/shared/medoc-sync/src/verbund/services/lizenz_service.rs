...
use crate::verbund::ports::{mark_provisioned, GeraetRepo, LizenzRepo, SqliteVerbundRepos};
...
/// Whether the local install may bind the verbund network listener.
pub fn verbund_network_ready(status: &VerbundStatus) -> bool {
    if status.is_owner {
        status.licensed
    } else {
...
pub fn needs_verbund_onboarding(status: &VerbundStatus) -> bool {
...
/// When keys were pre-imported via [`crate::verbund::activation::import_owner_activation`],
...
pub async fn verbund_status(pool: &SqlitePool) -> Result<VerbundStatus, AppError> {
    ...
    let identity = if lizenz.is_some() {
    ...
    let (cluster_id, seat_usage, is_owner) = if let Some(ref l) = lizenz {
        ...
        if !is_owner {
            if let Ok(device_id) = license_repo::ensure_device_id(pool).await {
                if let Some(g) =
                    crate::verbund::repo::find_by_device_id(pool, &device_id, &l.cluster_id).await?
                {
                    needs_reprovision = !g.identity_complete();
...
mod tests {
    ...
    use crate::master_keys;
    use crate::verbund::enums::{GeraetStatus, SeatRolle};
    use crate::verbund::ports::SqliteVerbundRepos;
    ...
    async fn activate_preserves_imported_cluster_and_keys() {
        std::env::set_var("MEDOC_VERBUND_DEVICE_SECRET", TEST_DEVICE_SECRET);
        ...
        let before = verbund_status(&pool).await.expect("status");
    ...
    #[test]
    fn verbund_network_ready_matrix() {
        let cases = [
    ...
    #[test]
    fn needs_verbund_onboarding_owner_requires_license() {
        let imported_owner = VerbundStatus {
            licensed: false,
            provisioned: false,
            is_owner: true,
        ...
        assert!(needs_verbund_onboarding(&imported_owner));

        let member_provisioned = VerbundStatus {
            provisioned: true,
            is_owner: false,
            ..imported_owner.clone()
        };
        assert!(!needs_verbund_onboarding(&member_provisioned));
    }
}
