...
use crate::verbund::entities::SeatCertificate;
use crate::verbund::enums::SeatRolle;
...
mod tests {
    ...
    fn seat_cert_roundtrip() {
        ...
        let (cert, token) =
            mint_seat_certificate(&sk, "cluster-1", &[1u8; 32], SeatRolle::Member, "lic-ref")
                .expect("mint");
        let parsed = verify_seat_certificate(&token, &pk).expect("verify");
        assert_eq!(parsed.cluster_id, cert.cluster_id);
        assert_eq!(parsed.seat_role, SeatRolle::Member);
    }
}
