...
pub async fn create(pool: &SqlitePool, data: &CreateLeistung) -> Result<Leistung, AppError> {
    ...
    let inserted = sqlx::query_as::<_, Leistung>("SELECT * FROM leistung WHERE id = ?1")
        .bind(&id)
        .fetch_one(pool)
        .await?;
...
pub async fn update(
    ...
    let updated = sqlx::query_as::<_, Leistung>("SELECT * FROM leistung WHERE id = ?1")
        .bind(id)
        .fetch_one(pool)
        .await?;
    let body = serde_json::to_string(&updated).unwrap_or_else(|_| format!("{{\"id\":\"{id}\"}}"));
    crate::infrastructure::database::sync_outbox::record_or_noop(
        pool, "leistung", id, "UPDATE", &body,
    )
    .await?;
    Ok(updated)
}
...
pub async fn delete(pool: &SqlitePool, id: &str) -> Result<(), AppError> {
    ...
    sqlx::query("UPDATE leistung SET aktiv = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?1")
        ...
        .execute(pool)
        .await?;
    if let Ok(updated) = sqlx::query_as::<_, Leistung>("SELECT * FROM leistung WHERE id = ?1")
        .bind(id)
        .fetch_one(pool)
        .await
    {
