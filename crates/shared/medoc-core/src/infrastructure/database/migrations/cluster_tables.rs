...
pub async fn ensure_cluster_tables(pool: &SqlitePool) -> Result<(), AppError> {
    ...
    // Extend sync_device → device registry (additive columns).
    for (table, col, ddl) in [
        ...
        (
            "sync_device",
            "device_status",
            "ALTER TABLE sync_device ADD COLUMN device_status TEXT CHECK (device_status IN ('PENDING','ACTIVE','REVOKED','BLOCKED'))",
        ),
        (
            "sync_device",
            "seat_cert",
        ...
        (
            "pairing_request",
            "pairing_state",
            "ALTER TABLE pairing_request ADD COLUMN pairing_state TEXT",
        ),
        (
            "pairing_request",
            "sas_hash",
    ...
    sqlx::query(
        "UPDATE sync_device SET device_status = 'ACTIVE'
         WHERE device_status IS NULL AND is_local = 1",
    )
    .execute(pool)
    .await
    .map_err(AppError::Database)?;
