...
pub async fn list_chart_validation(
    pool: State<'_, SqlitePool>,
    session_state: State<'_, SessionState>,
    patient_id: String,
) -> Result<Vec<ChartValidationRowDto>, AppError> {
    let _session = rbac::require(&session_state, "patient.read")?;
    let rows = chart_validation_repo::list_for_patient(&pool, &patient_id).await?;
    Ok(rows
        .into_iter()
        .map(|r| ChartValidationRowDto {
...
#[tauri::command]
#[tracing::instrument(level = "info", skip(pool, session_state))]
pub async fn set_chart_section_validated(
    pool: State<'_, SqlitePool>,
    session_state: State<'_, SessionState>,
    patient_id: String,
    section: String,
    ...
    chart_validation_repo::upsert_row(&pool, &patient_id, &section, &at, Some(by)).await?;

    // Master-data validation also covers anamnesis on the UI side.
    if section == "master" {
        chart_validation_repo::upsert_row(&pool, &patient_id, "anamnesis", &at, Some(by)).await?;
    }
...
#[tauri::command]
#[tracing::instrument(level = "info", skip(pool, session_state))]
pub async fn set_chart_item_validated(
    pool: State<'_, SqlitePool>,
    session_state: State<'_, SessionState>,
    patient_id: String,
    item_key: String,
    ...
    let at = chrono::Utc::now().to_rfc3339();
    let by = validated_by.as_deref().unwrap_or(&session.user_id);
    chart_validation_repo::upsert_row(&pool, &patient_id, &item_key, &at, Some(by)).await?;
    audit_repo::create(
        &pool,
        &session.user_id,
        "UPDATE",
...
#[tauri::command]
#[tracing::instrument(level = "info", skip(pool, session_state))]
pub async fn clear_chart_validation(
    pool: State<'_, SqlitePool>,
    session_state: State<'_, SessionState>,
    patient_id: String,
    ...
    if let Some(ref s) = section_or_item {
        ...
        n += chart_validation_repo::clear_for_patient(&pool, &patient_id, None).await?;
    }
    audit_repo::create(
        &pool,
        &session.user_id,
...
#[macro_export]
macro_rules! register_chart_validation_commands {
    () => {
