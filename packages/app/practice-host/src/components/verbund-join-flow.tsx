...
export function VerbundJoinFlow({ onComplete, onBack, completeWithLogin = false }: VerbundJoinFlowProps) {
    ...
    const join = async (admin: JoinAdminTarget) => {
        ...
        try {
            ...
            toast(`${t("page.lan.verbund_join.toast.connect_failed")} ${errorMessage(e)}`, "error");
        } finally {
            setJoinBusy(false);
        }
    };
    ...
    return (
        <div className="verbund-join-flow">
            {phase === "scan" && (
                <>
                    {scanBusy ? (
                    ...
                    <div className="onboarding-actions onboarding-actions--row">
                        <Button type="button" disabled={busy} onClick={() => void scan()}>
                            {scanBusy ? t("page.lan.verbund_join.scanning") : t("page.lan.verbund_join.scan_network")}
                        </Button>
                ...
                <>
                    ...
                    <div className="onboarding-actions onboarding-actions--row">
                        <Button type="button" disabled={busy || !manualHost.trim()} onClick={connectManual}>
                            {joinBusy ? t("page.lan.verbund_join.connecting") : t("page.lan.verbund_join.manual_connect")}
                        </Button>
                        <Button
                            type="button"
                            variant="secondary"
            ...
            {phase === "wait" ? <p role="status">{t("page.lan.verbund_join.provisioning_wait")}</p> : null}
        </div>
    );
}
