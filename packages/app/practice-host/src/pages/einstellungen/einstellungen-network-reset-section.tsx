...
import {
    verbundClusterResetExecute,
    verbundClusterResetPreview,
...
export function EinstellungenNetworkResetSection({ canOpsSystem }: Props) {
    const t = useT();
    const tp = useTParams();
    ...
    const loadPreview = useCallback(() => {
        if (!visible) return;
        void verbundClusterResetPreview()
            .then(setPreview)
    ...
    const execute = async () => {
        ...
        try {
            ...
            await verbundClusterResetExecute({
                mode,
                password,
