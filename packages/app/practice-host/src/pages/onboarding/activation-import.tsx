...
import {
    ...
    clusterImportActivation,
    clusterPickActivationManifest,
...
export function AktivierungImportOnboardingPage() {
    ...
    const browseManifest = async () => {
        try {
            const picked = await clusterPickActivationManifest();
    ...
    const importManifest = async () => {
        ...
        setBusy(true);
        try {
            const result = await clusterImportActivation(manifestPath.trim(), passphrase);
        ...
        <OnboardingShell>
            ...
            <div className="onboarding-actions onboarding-actions--row">
                ...
                {fingerprint && !importDone ? (
                    <Button type="button" variant="primary" onClick={() => navigate("/onboarding/subscription")}>
