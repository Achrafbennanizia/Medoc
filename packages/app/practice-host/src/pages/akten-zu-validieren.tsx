...
function akteStatusLabel(status: string, t: (key: string) => string): string {
    const key = `enum.akten_status.${status.trim().toLowerCase()}`;
    const label = t(key);
            ...
            <WorkspacePageHeader
                ...
                actions={(
                    <Button type="button" variant="secondary" size="sm" disabled={loading} onClick={() => void load()}>
                        {t("page.akten_zu_validieren.refresh")}
                    </Button>
                )}
            />
                            ...
                            <tbody>
                                {rows.map((r) => (
                                    <tr key={r.patient_id}>
                                        ...
                                        <td>
                                            <Badge variant={r.akte_status === "ENTWURF" ? "warning" : "default"}>
                                                {akteStatusLabel(r.akte_status, t)}
                                            </Badge>
                                        </td>
                                        ...
                                        <td>
                                            <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
                                                <Link
                                                    to={`/patienten/${r.patient_id}`}
                                                    className="btn btn-subtle"
                                                    style={{ padding: "5px 10px", fontSize: 12 }}
                                                >
                                                    {t("page.akten_zu_validieren.open_akte")}
                                                </Link>
                                                <Button
                                                    size="sm"
                                                    variant="primary"
