...
export async function listVertraegeFromBackend(): Promise<VertragItem[]> {
    const rows = await practiceSystem.invoke<VertragDto[]>("list_vertraege");
    return rows.map(dtoToItem);
}

export async function upsertVertragOnBackend(v: VertragItem): Promise<void> {
    await practiceSystem.invoke<void>("upsert_vertrag", { data: itemToDto(v) });
}

export async function deleteVertragOnBackend(id: string): Promise<void> {
    await practiceSystem.invoke<void>("delete_vertrag", { id });
}
...
export async function openVertragDokument(vertragId: string): Promise<void> {
    return practiceSystem.invoke<void>("open_vertrag_dokument", { vertragId });
}
