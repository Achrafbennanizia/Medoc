...
function preferenceAppliesToPerson(p: PlanPreference, staffId: string): boolean {
    if (p.staffIds.length === 0) return true;
    return p.staffIds.includes(staffId);
}
...
export function resolveSegmentsForPersonDay(
    ...
    for (const p of prefs) {
        if (!preferenceAppliesToPerson(p, staffId) || !preferenceAppliesToDate(p, ymdStr)) {
            continue;
        }
...
export function buildNetWorkForDay(
    ...
    const resolved = resolveSegmentsForPersonDay(staffId, ymdStr, prefs);
