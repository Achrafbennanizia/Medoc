/**
 * Additive WorkPlan entries (add + time off) and resolution to target intervals per day.
 */
import { eachDayOfInterval, format, getISODay, parseISO } from "date-fns";

export type WorkPlanComposeEntry =
    | {
        id: string;
        kind: "add_range";
        staffId: string;
...
export function composeWorkMinutesForDay(
    ...
    return resolveComposeWorkIntervals(staffId, ymdStr, entries).reduce((s, [a, b]) => s + (b - a), 0);
}
...
export function parseComposeEntries(raw: unknown): WorkPlanComposeEntry[] {
    if (!Array.isArray(raw)) return [];
    const out: WorkPlanComposeEntry[] = [];
    for (const x of raw) {
