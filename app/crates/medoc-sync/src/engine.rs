...
use crate::repo::{
    ensure_local_device, list_entries_since, list_pending_outbox, load_deployment,
    mark_outbox_delivered, save_deployment, status_snapshot, SyncStatusSnapshot,
};
...
impl SyncEngine {
    ...
    /// **BEST-EFFORT** mesh fan-out: replica fetches signed peer list from
    /// master, then pushes its pending outbox to every accepted peer that
    /// advertises a `peer_base_url` in `sync_device`.
    ///
    /// Gated by `deployment.unstable_mesh` to keep the default path simple.
    /// Live two-replica verification is **DEFERRED** — this is scaffolding
    /// so per-peer push reuses the same auth contract (`activation_token`)
    /// the master accepts.
    pub async fn run_mesh_sync(pool: &SqlitePool) -> Result<MeshSyncReport, AppError> {
        ...
        for peer in peers {
            ...
            match push_to_peer(pool, base, &bearer_token, &local_id, &pending).await {
    ...
    pub async fn run_replica_sync(pool: &SqlitePool) -> Result<SyncRunReport, AppError> {
        ...
        let deployment = load_deployment(pool).await?;
        if deployment.unstable_mesh {
            if let Err(e) = Self::run_mesh_sync(pool).await {
...
async fn push_to_peer(
    ...
    // Per-peer outbox delivery markers are not tracked yet (BEST-EFFORT);
    // the canonical master push still owns delivered_at.
    let _ = pool;
    Ok(result)
}
