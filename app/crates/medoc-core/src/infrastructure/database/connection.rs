...
async fn run_rust_only_migrations(pool: &SqlitePool) -> Result<(), AppError> {
    ...
    ensure_sync_replication_tables(pool).await?;

    Ok(())
}

/// Peer replication DDL (serverless mode). Kept in medoc-core so migrations do not depend on medoc-sync.
async fn ensure_sync_replication_tables(pool: &SqlitePool) -> Result<(), AppError> {
    sqlx::query(
