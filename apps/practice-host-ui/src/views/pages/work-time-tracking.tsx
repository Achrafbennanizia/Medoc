...
import { loadWorkPlanStore, weekDaysMonFirst, weekStartMonday, ymd } from "@/lib/staff-work-plan";
...
export function WorkTimeTrackingPage() {
    const t = useT();
    const tp = useTParams();
    const dateFnsLocale = useDateFnsLocale();
    const staffId = useAuthStore((s) => s.session?.user_id ?? "");
    ...
    return (
        <div className="practice-workspace-page animate-fade-in work-time-page">
            <WorkspacePageHeader
                title={t("page.workTime.title")}
                subtitle={t("page.workTime.tracking.subtitle")}
            />

            <div className="work-time-page__grid">
            <Card className="work-time-today-card">
                <CardHeader title={t("page.workTime.today")} />
                <div className="card-pad work-time-today-body">
                    ...
                    <p className="work-time-today-status">
                        {t("page.workTime.status.label")}{" "}
                        <strong>
                            {!session
                                ? t("page.workTime.status.none")
                                : session.status === "PAUSED"
                                  ? t("page.workTime.status.paused")
                                  : t("page.workTime.status.active")}
                        </strong>
                        {session?.status === "PAUSED" && session.pauseStartedAt ? (
                            <span className="work-time-today-since">
                                {tp("page.workTime.status.since", {
                    ...
                    <p className="work-time-today-week-total">
                        {tp("page.workTime.week_finished", {
            ...
            <Card className="work-time-week-card">
                <CardHeader
                    title={t("page.workTime.week_overview")}
                    action={
                        ...
                        return (
                            <div key={day.date} className="work-time-week-col">
                                ...
                                <div
                                    ...
                                    title={tp("page.workTime.week_bar_tooltip", {
                                        date: day.date,
                ...
                <div className="work-time-week-legend card-pad">
                    <span className="work-time-week-legend__item">
                        ...
                        {t("page.statistics.workTime.week_actual")}
                    </span>
                    <span className="work-time-week-legend__item">
                        <span className="work-time-week-legend__swatch work-time-week-legend__swatch--soll" aria-hidden />
                        {t("page.work_plan.soll_table.col.work")}
                    </span>
                </div>
                <p className="card-pad work-time-week-footer">
                    {t("page.workTime.footer.soll_hint")}{" "}
                    <Link to="/staff/work-plan">{t("page.workTime.footer.open_plan")}</Link>
                    {" · "}
                    <Link to="/settings">{t("page.workTime.footer.settings")}</Link>
                </p>
            </Card>
            </div>
        </div>
