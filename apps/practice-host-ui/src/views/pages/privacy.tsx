...
/**
 * Privacy page — GDPR Art. 15 (access), 17 (erasure), 20 (portability).
 * Bundles patient export (JSON) and deletion request (pseudonymization with retention
 * klinischer Daten until Ablauf §630f BGB / 30 Jahre).
 */
const PAGE_SIZE = 50;
...
export function PrivacyPage() {
    ...
    const resolveDsgvoExport = useCallback(async () => {
        ...
        return {
            exportTitle: t("page.privacy.dialog_preview_title"),
            hint: tp("page.privacy.export_bundle_hint", { name: exportPatient.name }),
    ...
    async function handleErase() {
        ...
        try {
            ...
            toast(t("page.privacy.toast_erase_done"));
            setConfirmErase(null);
            await loadPatients();
        } catch (e) {
            toast(`${t("page.privacy.toast_error_prefix")} ${errorMessage(e)}`);
        } finally {
            setBusyId(null);
        }
    }
    ...
    return (
        <div className="practice-workspace-page animate-fade-in">
            <WorkspacePageHeader
                ...
                back={{ to: "/settings", label: t("page.privacy.back_settings") }}
                subtitle={
                    <>
                        {t("page.privacy.intro_prefix")}
                        <em>{t("page.privacy.intro_em")}</em>
                        {t("page.privacy.intro_suffix")}
                    </>
                }
            />

            <Card className="card-pad">
                <CardHeader title={t("page.privacy.card_search")} />
                {patientsLoading ? (
                    <p className="text-body text-on-surface-variant" role="status">{t("page.privacy.loading")}</p>
                ) : patientsError ? null : (
                    <PatientComboField
                        id="dsgvo-patient-filter"
                        label={t("page.privacy.filter_label")}
                        patients={patients}
                        patientId={patientPickId}
                        onPatientIdChange={setPatientPickId}
                        onQueryChange={setFilter}
                        placeholder={t("page.privacy.filter_ph")}
                    />
                )}
            </Card>

            {patientsLoading ? (
                <PageLoading label={t("page.privacy.loading")} />
            ) : patientsError ? (
                <PageLoadError message={patientsError} onRetry={() => void loadPatients()} />
            ) : filtered.length === 0 ? (
                <EmptyState icon="👥" title={t("page.privacy.empty_title")} description={t("page.privacy.empty_desc")} />
            ) : (
                <div className="card tbl-data-card">
                    ...
                    <table className="tbl tbl-fluid">
                        <thead>
                            <tr>
                                <th>{t("page.privacy.col.name")}</th>
                                <th>{t("page.privacy.col.status")}</th>
                                <th>{t("page.privacy.col.actions")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {pageRows.map((p) => (
                            ...
                            <div className="row" style={{ justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
                                <span style={{ color: "var(--fg-3)", fontSize: 13 }}>
                                    {totalFiltered <= PAGE_SIZE
                                        ? formatTpl(t("page.privacy.page_all"), { count: totalFiltered })
                                        : formatTpl(t("page.privacy.page_range"), {
            ...
            <ConfirmDialog
                ...
                title={t("page.privacy.erase_title")}
                message={confirmErase ? formatTpl(t("page.privacy.erase_confirm"), { name: confirmErase.name }) : ""}
                confirmLabel={t("page.privacy.erase_confirm_btn")}
                danger
            ...
            {eraseReport && (
                <Card className="card-pad">
                    <CardHeader title={t("page.privacy.report_title")} />
                    <ul className="text-body" style={{ margin: 0, paddingInlineStart: 20, display: "flex", flexDirection: "column", gap: 6 }}>
                        <li>{t("page.privacy.report_patient_id")} <span style={{ fontFamily: "ui-monospace, monospace" }}>{eraseReport.patient_id}</span></li>
                        <li>{t("page.privacy.report_anonymised")} {eraseReport.anonymised_at}</li>
                        <li>{t("page.privacy.report_records")} {eraseReport.deleted_records}</li>
                    </ul>
                </Card>
            )}
            <DataExportPickerDialog
                ...
                title={t("page.privacy.export_title")}
                description={t("page.privacy.export_desc")}
