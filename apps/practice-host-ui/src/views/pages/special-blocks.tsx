...
import type { PracticeClosureMode, PracticeClosureRule } from "@/lib/practice-planning";
import { loadPracticeWorkHoursConfig, savePracticeWorkHoursConfig } from "@/lib/practice-planning";
import { errorMessage } from "@/lib/utils";
import { useRbac } from "@/lib/use-rbac";
import { EditIcon, TrashIcon } from "@/lib/icons";

export function SpecialBlockedTimesPage() {
    const t = useT();
    const tp = useTParams();
    const toast = useToastStore((s) => s.add);
    const { canWritePracticePlanning } = useRbac();
    const [closures, setClosures] = useState<PracticeClosureRule[]>([]);
    const [loading, setLoading] = useState(true);
    const [closureDate, setClosureDate] = useState("");
    const [closureMode, setClosureMode] = useState<PracticeClosureMode>("FULL_DAY");
    ...
    const [selected, setSelected] = useState<PracticeClosureRule | null>(null);
    ...
    const fillFormFrom = (r: PracticeClosureRule) => {
    ...
    const saveSperrEdit = () => {
        ...
        if (closureMode === "CUSTOM" && closurePeriods.some((p) => !p.from || !p.to || p.from >= p.to)) {
            toast(t("page.special_blocked_times.custom_periods_error"), "error");
            return;
        }
        const updated: PracticeClosureRule = {
            ...selected,
            date: closureDate,
            mode: closureMode,
        ...
        setSperrEdit(false);
        setSelected(updated);
        toast(t("page.special_blocked_times.toast.saved"));
    };

    const selectRow = (r: PracticeClosureRule) => {
        setSelected(r);
        setCreating(false);
        setSperrEdit(false);
    };
    ...
    const addClosure = () => {
        ...
        setSelected(row);
        setCreating(false);
        toast(t("page.special_blocked_times.toast.added"));
    };
    ...
    const addNextDayEmergencyClose = () => {
        ...
        const row: PracticeClosureRule = {
            ...
            mode: "FULL_DAY",
            periods: [],
            reason: t("page.special_blocked_times.emergency_reason"),
        };
        const next = [row, ...closures].sort((a, b) => a.date.localeCompare(b.date));
        void saveClosures(next);
        setSelected(row);
        setCreating(false);
        toast(t("page.special_blocked_times.toast.next_day"));
    };
    ...
    const timeLabel = (r: PracticeClosureRule) =>
        r.mode === "CUSTOM" ? (r.periods ?? []).map((p) => `${p.from}–${p.to}`).join(", ") : "—";

    const modeLabel = (mode: PracticeClosureMode, short = false) =>
        mode === "FULL_DAY"
            ? t(short ? "page.special_blocked_times.mode.full_day_short" : "page.special_blocked_times.mode.full_day")
            : t(short ? "page.special_blocked_times.mode.custom_short" : "page.special_blocked_times.mode.custom");
        ...
        <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Select
                    ...
                    label={t("common.type")}
                    value={closureMode}
                    onChange={(e) => setClosureMode(e.target.value as PracticeClosureMode)}
                    options={[
                        { value: "FULL_DAY", label: t("page.special_blocked_times.mode.full_day") },
                        { value: "CUSTOM", label: t("page.special_blocked_times.mode.custom") },
                    ]}
                />
                    ...
                    <div className="col" style={{ gap: 8, gridColumn: "1 / -1" }}>
                        {closurePeriods.map((p, idx) => (
                            <div key={`period-${idPrefix}-${idx}`} className="row" style={{ gap: 8, alignItems: "end", flexWrap: "wrap" }}>
                                <Input
                                    ...
                                    type="time"
                                    label={tp("page.special_blocked_times.from_period", { n: idx + 1 })}
                                    value={p.from}
                                ...
                                <Input
                                    id={`${idPrefix}-to-${idx}`}
                                    type="time"
                                    label={tp("page.special_blocked_times.to_period", { n: idx + 1 })}
                                    value={p.to}
                        ...
                        <div>
                            <Button type="button" size="sm" variant="secondary" onClick={addPeriod}>
                                {t("page.special_blocked_times.add_period")}
                            </Button>
                        </div>
                    </div>
                ) : null}
    ...
    const sidePanel = (() => {
        if (creating) {
            return (
                <Card className="products-detail-card">
                    <CardHeader
                        title={t("page.special_blocked_times.new_rule.title")}
                        subtitle={t("page.special_blocked_times.new_rule.subtitle")}
                        action={
                            <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
                                <Button type="button" size="sm" variant="danger" onClick={addNextDayEmergencyClose}>
                                    {t("page.special_blocked_times.close_practice_tomorrow")}
                                </Button>
                    ...
                    <div className="card-pad" style={{ paddingTop: 0, display: "flex", flexDirection: "column", gap: 12 }}>
                        <p style={{ fontSize: 12.5, color: "var(--fg-3)", margin: 0 }}>{t("page.special_blocked_times.form.hint")}</p>
                        {closureFormFields("closure")}
                        <div className="row" style={{ justifyContent: "flex-end", gap: 8 }}>
                            <Button type="button" variant="secondary" onClick={() => void addClosure()}>
                                {t("page.special_blocked_times.add_closure")}
                            </Button>
                        </div>
                    </div>
                </Card>
        ...
        if (selected && sperrEdit) {
            return (
                <Card className="products-detail-card">
                    <CardHeader
                        title={t("page.special_blocked_times.edit.title")}
                        subtitle={t("page.special_blocked_times.edit.subtitle")}
                        action={
                            <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
        ...
        return (
            <Card className="card-pad products-detail-card products-detail-card--empty">
                <p style={{ margin: 0, color: "var(--fg-3)", fontSize: 14, lineHeight: 1.5 }}>
                    {t("page.special_blocked_times.empty.hint")}
                </p>
            </Card>
        );
    })();

    if (loading) return <PageLoading label={t("page.special_blocked_times.loading")} />;

    return (
        <div className="products-page practice-workspace-page animate-fade-in">
            <AdministrationPageHeader
                title={t("page.special_blocked_times.title")}
                subtitle={t("page.special_blocked_times.subtitle")}
                actions={
                    canWritePracticePlanning ? (
                        <Button type="button" variant={creating ? "secondary" : "primary"} onClick={creating ? closeForm : openForm}>
                            {creating ? t("page.special_blocked_times.close_form") : t("page.special_blocked_times.new_rule")}
                        </Button>
                    ) : null
                }
            />
                ...
                <div className="products-workspace__list">
                    {sorted.length === 0 ? (
                        <Card className="card-pad">
                            <EmptyState icon="🚫" title={t("page.special_blocked_times.empty.title")} description={t("page.special_blocked_times.empty.desc")} />
                        </Card>
                    ) : (
